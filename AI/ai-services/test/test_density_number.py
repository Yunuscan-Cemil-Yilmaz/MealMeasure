...
def main():
    ...
    print("\nResim sayısı:", len(imgs))
    try:
        index = int(input(f"Kaçıncı sıradaki resmi incelemek istersiniz? (1-{len(imgs)}, iptal için -1): "))
    except ValueError:
    ...
    if index == -1:
        ...
        print("Hatalı seçim: Negatif veya sıfır değer girilemez.")
        return
    elif index > len(imgs):
        print("Hatalı seçim: Bu sırada bir resim bulunmuyor.")
        return
